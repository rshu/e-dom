...
def DT():
    ...
    model = DecisionTreeClassifier(criterion=b, splitter=c, min_samples_split=a, max_features=None,
...
def RF():
    ...
    model = RandomForestClassifier(n_estimators=a, criterion=b, min_samples_split=c, max_features=None,
...
def SVM():
    ...
    tmp = str(a) + "_" + b + "_" + str(c) + "_" + str(round(d, 5)) + "_" + str(round(e, 5)) + "_" + str(
        round(f, 5)) + "_" + SVC.__name__
    return model, tmp
...
def run_model_farsec(train_data, test_data, model, metric, training=-1):
    model.fit(train_data[train_data.columns[:training]], train_data["label"])
    prediction = model.predict(test_data[test_data.columns[:training]])
    test_data.loc[:, "prediction"] = prediction
    return round(get_score(metric, prediction, test_data["label"].tolist(), test_data), 5)